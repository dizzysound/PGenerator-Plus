...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   const slot=key=>pgAutomationEl('Progress').querySelector('[data-automation-eta-'+key+']').textContent;
   check(slot('batch')==='15–30 min'&&slot('job')==='—'&&slot('job-label')==='this job left, estimating','batch time range sits in its own readout; the job slot stays put while unknown');
   run.time_estimate.scope='stage';pgAutomationRenderProgress();
   check(slot('job')==='15–30 min'&&slot('job-label')==='this stage left'&&slot('batch')==='—'&&pgAutomationEl('Progress').textContent.includes('Batch estimate still learning'),'stage ETA cannot masquerade as whole-batch remaining time');
   ...
   check(text.includes('Current job: about ')&&text.includes('Whole batch: about ')&&text.includes('similar jobs'),'current job and whole batch estimates are separate and explain approximate history');
   pgAutomationRenderProgress();
   check(slot('job')==='7–15 min'&&slot('batch')==='45 min–1.5 h'&&slot('job-label')==='this job left'&&slot('batch-label')==='whole batch left','readouts round outwards and label each range plainly');
   check(pgAutomationEstimateRange({low:3660,high:4200})==='1–1.5 h','hour rounding keeps a 61–70 minute prediction inside the displayed range');
   ...
   run.time_estimate.calculated_at=now;run.time_estimate.ranges={job:{low:1800,high:2400},batch:{low:5000,high:7000}};
   let measured=pgAutomationEstimateModel(run,now);
   check(measured.job.low===1800&&measured.job.high===2400,'UI uses empirical server bounds without applying a second percentage range');
   run.time_estimate.ranges.job={low:'invalid',high:null};
   measured=pgAutomationEstimateModel(run,now);
   check(Number.isFinite(measured.job.low)&&Number.isFinite(measured.job.high),'invalid saved bounds fall back to a finite estimate');
